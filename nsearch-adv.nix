# This is not mine but from a great dev
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    cp nixpkgs.sh "$out/bin/nsearch-adv"

    # Replace "all ctrl-a" with "all alt-a"
    sed -i 's/"all ctrl-a"/"all alt-a"/g' $out/bin/nsearch-adv

    # Make it executable
    chmod +x $out/bin/nsearch-adv

    # Wrap the script to ensure dependencies are available
    wrapProgram $out/bin/nsearch-adv \
